...
export async function saveCrm(kindInput:unknown,idInput:unknown,versionInput:unknown,input:unknown){
 ...
 }catch(error){return {ok:false as const,error:error instanceof z.ZodError?'Revisa los campos: '+error.issues.map(i=>i.path.join('.')+' — '+i.message).join('; '):'No fue posible guardar. Verifica tu sesión y el permiso CRM.'};}
}
