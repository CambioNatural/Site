...
export async function setUserPermissions(id:unknown,modules:unknown,expected:unknown){
 try{const input=z.object({id:z.uuid(),modules:z.array(z.enum(cmsModules)).max(cmsModules.length),expected:z.array(z.enum(cmsModules)).max(cmsModules.length)}).parse({id,modules,expected});
