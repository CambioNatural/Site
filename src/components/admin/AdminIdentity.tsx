...
type Module = 'home' | 'blog' | 'popups' | 'users';
...
export default function AdminIdentity({ active, dirty = false, pending = false, access }: { active?: Module; dirty?: boolean; pending?: boolean; access?:CmsAccess }) {
  return <div className="cms-identity">
    ...
    {active && <nav aria-label="Módulos del CMS">{modules.filter(item=>item.id==='users'?access?.admin:!access||access.modules.includes(item.id)).map(item => <Link key={item.id} href={item.href} aria-current={active === item.id ? 'page' : undefined} aria-disabled={pending || undefined} onClick={event => {
