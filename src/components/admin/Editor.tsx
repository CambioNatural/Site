...
export default function Editor(props: Props) {
    ...
    return <main className="cms-shell"><AdminIdentity active="home" access={props.access} dirty={dirty} pending={pending}/><header><div><h1>Portada</h1><p>Edita la portada, revisa el borrador y publica cuando esté listo.</p><small>{props.email}</small></div><form action={logout}><button className="cms-secondary" disabled={dirty || pending}>Cerrar sesión</button></form></header>

 <div className="cms-toolbar"><button disabled={pending || (!dirty && version > 0)} onClick={() => save()}>Guardar borrador</button><button className="cms-secondary" disabled={pending} onClick={() => save(true)}>{dirty || version === 0 ? 'Guardar y ver' : 'Ver borrador'}</button><button className="cms-publish" disabled={pending || dirty || version === 0 || version === published} onClick={() => { setConfirmPublish(true); setRestoreTarget(null); }}>Publicar cambios</button></div>
 <p className="cms-status" role="status" aria-live="polite">{pending ? 'Procesando…' : status || (dirty ? 'Guarda tus cambios para incluirlos en la vista previa.' : 'El borrador está guardado. Puedes revisarlo antes de publicar.')}</p>
 <div className="cms-version-line"><span>Borrador {version}</span><span>{published ? `Publicado: versión ${published}` : 'Sin publicación'}</span>{dirty && <strong>Cambios sin guardar</strong>}</div>
 {confirmPublish && <div className="cms-warning"><p>La versión {version} reemplazará el contenido visible del sitio conectado a este proyecto.</p><button disabled={pending} onClick={publish}>Confirmar publicación</button> <button className="cms-secondary" onClick={() => setConfirmPublish(false)}>Cancelar</button></div>}
 <div className="cms-grid"><nav className="cms-sections" aria-label="Secciones del contenido">{groups.map((group, index) => { const changed = group.fields.some(field => field.image ? read(document, `${field.path}.src`) !== read(savedDocument, `${field.path}.src`) || read(document, `${field.path}.alt`) !== read(savedDocument, `${field.path}.alt`) : read(document, field.path) !== read(savedDocument, field.path)); return <button key={group.label} className="cms-section-button" aria-current={activeGroup === index ? 'true' : undefined} onClick={() => setActiveGroup(index)}>{group.label}{changed && <small>Modificado</small>}</button>; })}</nav><div className="cms-edit-panel">{groups.filter((_, index) => index === activeGroup).map(group => <section key={group.label} aria-labelledby="cms-section-heading"><h2 id="cms-section-heading">{group.label}</h2><p className="cms-help">Los cambios se conservan al cambiar de sección. Guarda para actualizar el borrador.</p><fieldset disabled={pending}>{group.fields.map(field => field.image ? <div key={field.path} className="cms-image"><p>{field.label}</p>{/* Image is either a validated local asset or authenticated private endpoint. */}
 ...
 <label>Reemplazar {field.label.toLowerCase()}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => { const file = event.target.files?.[0]; event.target.value = ''; if (!file)
            ...
            } }); }}/></label><small>PNG, JPEG o WebP · máximo 5 MB. Se optimiza para la web. Describe las imágenes informativas; deja vacío el texto alternativo de las decorativas.</small><label>Texto alternativo · {field.label}<input maxLength={300} value={read(document, `${field.path}.alt`)} onChange={e => change(`${field.path}.alt`, e.target.value)}/></label></div> : <label key={field.path}>{field.label}{field.short ? <input value={read(document, field.path)} maxLength={field.path.endsWith('href') ? 2048 : 250} onChange={e => change(field.path, e.target.value)}/> : <textarea value={read(document, field.path)} maxLength={6000} onChange={e => change(field.path, e.target.value)}/>}<small className="cms-field-help">{field.path.endsWith('mobileText') ? 'Opcional. Si lo dejas vacío, se usa el texto principal.' : field.path.endsWith('href') ? 'Usa una dirección https o una ruta existente del sitio.' : `${read(document, field.path).length.toLocaleString('es-MX')} / ${field.short ? '250' : '6,000'} caracteres`}</small></label>)}</fieldset></section>)}</div>
 <aside className="cms-history"><h2>Historial</h2><p>Recupera una publicación como borrador para revisarla.</p>{!revisions.length && <p>Aquí aparecerán las versiones cuando publiques por primera vez.</p>}<ul>{revisions.map(rev => <li key={rev.version}><strong>Versión {rev.version}</strong>{rev.version === published && <span className="cms-live-label">Publicada</span>}<br /><time dateTime={rev.published_at}>{new Date(rev.published_at).toLocaleString('es-MX', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'America/Mexico_City' })}</time><br /><button className="cms-secondary" disabled={pending || dirty} onClick={() => { setRestoreTarget(rev.version); setConfirmPublish(false); }}>Recuperar versión {rev.version}</button>{restoreTarget === rev.version && <div className="cms-restore-confirm"><p>Se reemplazará el borrador guardado por la versión {rev.version}. La publicación actual se conserva.</p><button disabled={pending || dirty} onClick={() => start(async () => { try {
    ...
    } })}>Confirmar recuperación</button><button className="cms-secondary" disabled={pending} onClick={() => setRestoreTarget(null)}>Cancelar</button></div>}</li>)}</ul>{dirty && <p className="cms-help">Guarda tus cambios antes de recuperar una versión.</p>}</aside></div></main>;
}
