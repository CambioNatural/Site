...
export default function EntryManager({ kind, initialEntries, access }: {
 ...
 <div className="cms-collection"><aside className="cms-entry-list"><button disabled={pending} onClick={() => select(null)}>{popup ? 'Nuevo pop-up' : 'Nueva entrada'}</button><label>Buscar contenido<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Título o identificador"/></label>{entries.length === 0 && <p>Aún no hay contenido. Empieza con un borrador.</p>}{search&&!entries.some(e=>(e.content.title+' '+e.slug).toLowerCase().includes(search.toLowerCase()))&&<p>No hay contenido que coincida.</p>}<ul>{entries.filter(e=>(e.content.title+' '+e.slug).toLowerCase().includes(search.toLowerCase())).map(e => <li key={e.id}><button className="cms-secondary" disabled={pending} aria-current={selected?.id === e.id ? 'true' : undefined} onClick={() => select(e)}><strong>{e.content.title}</strong><small>{e.publishedVersion ? `Publicación ${e.publishedVersion}` : 'Borrador'}{e.publishedVersion && e.version !== e.publishedVersion ? ' · cambios pendientes' : ''}</small></button></li>)}</ul></aside>
