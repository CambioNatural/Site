...
import type {BlogContent} from '@/lib/entries/schema';
...
export default function EntryManager({ kind, initialEntries, access }: {
 ...
 const {t,locale}=useAdminLanguage();
    ...
    function select(entry: Entry | null) { if (dirty && !window.confirm(t("Hay cambios sin guardar. ¿Descartarlos para cambiar de contenido?")))
        return; const next = entry?.content ?? emptyEntry(kind); setEditorSession(n=>n+1); setSelected(entry); setContent(next); setSaved(next); setMessage(''); setConfirm(null); setPreview(false); }
    ...
    function run(action: 'save' | 'publish' | 'unpublish') { start(async () => { try {
        ...
        setConfirm(null);
        setMessage(action === 'save' ? 'Borrador guardado.' : action === 'publish' ? (popup ? 'Pop-up activado. Se mostrará según las fechas y páginas elegidas.' : 'Entrada publicada en el blog.') : 'Publicación retirada. El borrador se conserva.');
    }
    catch {
    ...
    return <main className="cms-shell"><AdminIdentity active={kind === 'blog' ? 'blog' : 'popups'} access={access} dirty={dirty} pending={pending}/><header><div><h1>{popup ? 'Pop-ups' : 'Blog'}</h1><p>{popup ? t("Crea avisos y programa cuándo y dónde aparecen.") : t("Prepara y publica entradas en el blog del sitio.")}</p></div></header>
 <div className="cms-toolbar">{!popup&&<><Link href="/admin/preview/blog" className="cms-secondary">{t("Ver diseño del blog")}</Link>{selected&&!dirty&&<Link href={`/admin/preview/blog/${selected.id}`} className="cms-secondary">{t("Ver borrador en el sitio")}</Link>}</>}<button disabled={pending || (!dirty && !!selected)} onClick={() => run('save')}>{t("Guardar borrador")}</button><button className="cms-secondary" disabled={pending} onClick={() => setPreview(v => !v)}>{preview ? t("Cerrar vista previa") : t("Vista previa local")}</button><button className="cms-publish" disabled={pending || dirty || !selected || selected.version === selected.publishedVersion} onClick={() => setConfirm('publish')}>{popup ? t("Activar pop-up") : t("Publicar entrada")}</button>{selected?.publishedVersion && <button className="cms-secondary" disabled={pending || dirty} onClick={() => setConfirm('unpublish')}>{popup ? t("Desactivar") : t("Retirar publicación")}</button>}</div>
 ...
 </fieldset>{preview && <article className="cms-content-preview" aria-label={t("Vista previa del contenido")}><p className="cms-help">{t("Vista previa local · incluye cambios sin guardar")}</p>{image && <><img src={image} alt={content.alt}/></>}<h2>{content.title || t("Título")}</h2>{popup?<p>{content.body || t('El contenido aparecerá aquí.')}</p>:<BlogBody body={content.body || t('El contenido aparecerá aquí.')} richText={(content as BlogContent).richText}/>}{popup && (content as PopupContent).buttonLabel && <span className="cms-preview-cta">{(content as PopupContent).buttonLabel}</span>}</article>}</section></div></main>;
}
